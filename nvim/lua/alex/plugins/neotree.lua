		...
		require("neo-tree").setup({
			...
			commands = {
				duplicate_dir = function(state)
					...
					local src = node.path
					local new_name = vim.fn.input("Duplicate as: ", "", "file")
					if new_name ~= "" then
						local dst = vim.fn.fnamemodify(src, ":h") .. "/" .. new_name
						local ok = os.execute(string.format('cp -r "%s" "%s"', src, dst))
						if ok then
							vim.notify(string.format("Duplicated %s → %s", src, dst), vim.log.levels.INFO)
